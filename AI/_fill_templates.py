...
LABEL_FILLERS = [
    (re.compile(r"(?:H[oọ]?\s*(?:và)?\s*t[eê]n|BÊN\s*B|T[eê]n\s*NV|[ÔO]ng\s*/\s*[Bb][àa])\s*:", re.I), fake_name),
    (re.compile(r"[ĐD][iị]a\s*ch[iỉ]\s*:", re.I), fake_address),
    (re.compile(r"[Ss][ốo]\s*CCCD|CCCD\s*/\s*H[ộo]\s*chi[ếe]u", re.I), fake_cccd),
    (re.compile(r"[Đđ]i[ệe]n\s*tho[ạa]i\s*:", re.I), fake_phone),
    (re.compile(r"M[ứu]c\s*l[ưu][ơo]ng(?:\s*ch[íi]nh)?\s*:", re.I), fake_salary),
    (re.compile(r"[Cc]h[ứu]c\s*danh|[Cc]h[ứu]c\s*v[ụu]|[Vv][ịi]\s*tr[íi]\s*c[ôo]ng\s*t[áa]c", re.I), fake_position),
    (re.compile(r"BÊN\s*A\s*:\s*C[ÔO]NG\s*TY", re.I), fake_company),
]
...
LABEL_FILLERS_BARE = [
    (re.compile(r"H[oọ]?\s*[Vv][àa]?\s*T[eê]n|BÊN\s*B|T[eê]n\s*NV|[ÔO]ng\s*/\s*[Bb][àa]", re.I), fake_name),
    (re.compile(r"[ĐD][iị]a\s*ch[iỉ]", re.I), fake_address),
    (re.compile(r"[Ss][ốo]\s*CCCD|CCCD\s*/\s*H[ộo]\s*chi[ếe]u|CMND", re.I), fake_cccd),
    (re.compile(r"[Đđ]i[ệe]n\s*tho[ạa]i", re.I), fake_phone),
    (re.compile(r"M[ứu]c\s*l[ưu][ơo]ng(?:\s*ch[íi]nh)?|L[ưu][ơo]ng\s*ch[íi]nh|L[ưu][ơo]ng\s*c[ơo]\s*b[ảa]n", re.I), fake_salary),
    (re.compile(r"[Cc]h[ứu]c\s*danh|[Cc]h[ứu]c\s*v[ụu]|[Vv][ịi]\s*tr[íi]\s*c[ôo]ng\s*t[áa]c", re.I), fake_position),
    (re.compile(r"M[ãa]\s*Nh[âa]n\s*Vi[êe]n", re.I), lambda: str(random.randint(1000, 9999))),
]
...
def _try_fill_paragraph_text(text: str) -> tuple[str, bool]:
    for label_re, generator in LABEL_FILLERS:
        m = label_re.search(text)
        if not m:
            continue
        after = text[m.end():]
        if _is_placeholder(after):
            return text[:m.end()] + " " + generator(), True
    return text, False
...
def fill_docx(src_path: str, dst_path: str, seed: int | None = None) -> int:
    if seed is not None:
        random.seed(seed)
        Faker.seed(seed)
    ...
    for p in doc.paragraphs:
        new_text, did_fill = _try_fill_paragraph_text(p.text)
        if did_fill:
    ...
    for table in doc.tables:
        for row in table.rows:
            ...
            for i, cell in enumerate(cells):
                ...
                new_text, did_fill = _try_fill_paragraph_text(cell.text)
                ...
                for label_re, generator in LABEL_FILLERS_BARE:
                ...
                if i + 1 < len(cells) and _is_placeholder(cells[i + 1].text):
                    ...
                    if target.paragraphs:
                        _set_paragraph_text(target.paragraphs[0], matched_generator())
                        filled += 1
