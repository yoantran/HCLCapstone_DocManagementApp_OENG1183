...
LABEL_PATTERNS = {
    ...
    "name": re.compile(r"Employee(?:\s*Name)?\s*:[ \t]*(.+)", re.IGNORECASE),
    "address": re.compile(r"Employee\s*Address\s*:[ \t]*(.+)", re.IGNORECASE),
    "bsb": re.compile(r"BSB\s*:[ \t]*(.+)", re.IGNORECASE),
    "account_number": re.compile(r"Account\s*:[ \t]*(.+)", re.IGNORECASE),
}

INCOME_LABEL_PATTERNS = {
    "gross": re.compile(r"(?:Total\s*gross\s*payment|Gross\s*Pay)\s*:?[ \t]*(.+)", re.IGNORECASE),
    "net": re.compile(r"(?:NET\s*PAY|Total\s*net\s*payment|Net\s*Pay)\s*:?[ \t]*(.+)", re.IGNORECASE),
}
